...
*** （参考）HEADによるコミットの指定
- 特定のコミットのIDを指定する方法に「HEAD」を使った相対指定があるが，
  なれないと間違いやすい
  - showコマンドで確認しながら用いると良い
...
- 「git add .」コマンドを実行すると，Gitは削除や名前の変更も
  自動的に検知する
  - 参考：古いバージョンのgitでは「git rm」や「git mv」コマンドを利用する
    必要があった
*** Gitに無視させたいファイル
...
- 次の手順で作成する
  ...
  - 「Initialize this repository with a README」に _チェックを入れる_ こと
...
*** GitHub Flowに習熟するには？
- ここで手順2:に戻り，一連の作業を複数回（5回以上!）繰り返すこと
  - _体に叩き込む！_

*** 参考：git fetch
- git fetchはリモートにあるブランチの最新情報を
  ローカルに取ってくるコマンド
- git pullはmasterブランチで次の２つを実行することと同じ意味
