...
\subsection{コンフリクト}
\label{sec-1-7}
\subsubsection{コンフリクトとは}
\label{sec-1-7-1}
\begin{itemize}
\item ファイルの同じ箇所を，異なる内容に編集すると発生する
\item Gitはどちらの内容が正しいのかわからない
\item 次のシナリオに従い，コンフリクトを発生させてみよう
\end{itemize}

\subsubsection{コンフリクトのシナリオ}
\label{sec-1-7-2}
\begin{itemize}
\item 「のび太」の作業
\begin{itemize}
\item nobitaブランチを作成する
\item READMEファイルの \uline{一行目} を「Nobita's README.」に変更する
\item 変更をaddしてcommitする
\end{itemize}
\item 一度，masterブランチにもどる
\begin{itemize}
\item READMEは変わってない
\end{itemize}
\item 「しずか」の作業
\begin{itemize}
\item shizukaブランチを作成する
\item READMEファイルの \uline{一行目} を「Shizuka's file.」に変更する
\item 変更をaddしてcommitする
\end{itemize}
\end{itemize}

\subsubsection{マージとコンフリクト発生}
\label{sec-1-7-3}
\begin{itemize}
\item masterブランチに移動
\item まず，nobitaブランチをマージ
\begin{itemize}
\item 問題なくマージできる
\end{itemize}
\item 次に，shizukaブランチをマージ
\begin{itemize}
\item ここでコンフリクトが発生する
\end{itemize}
\end{itemize}

\subsubsection{コンフリクト時のメッセージ}
\label{sec-1-7-4}
\begin{itemize}
\item mergeに失敗するとようなメッセージが出る（長いので改行を加えた）
\end{itemize}

\begin{minted}[frame=single,linenos=true]{text}
Auto-merging README
CONFLICT (content):
...
\begin{itemize}
\item また，git statusするとUnmerged pathes:の欄に，「both modified: README」
と表示される
\end{itemize}

\subsubsection{READMEファイルの内容}
\label{sec-1-7-5}
\begin{itemize}
\item READMEを開くとコンフリクトが起きた箇所がわかる
\end{itemize}
...
\begin{itemize}
\item =======の上がマージ前のmasterブランチ，
下がマージしようとしたshizukaブランチの内容
\end{itemize}

\subsubsection{コンフリクトの解消}
\label{sec-1-7-6}
\begin{itemize}
\item テキストエディタで修正し，手動でコンフリクトを解消する
\end{itemize}
...
\subsubsection{解消した結果をコミットする}
\label{sec-1-7-7}
\begin{itemize}
\item その後はいつもどおり，addしてcommitすれば作業が継続できる
\begin{itemize}
\item マージ済みのmasterから新しくブランチを作成すること
\end{itemize}
\item なお， \uline{テキストエディタを用い，手動で正しくコンフリクトを解消する前でも
commitできてしまう} ので，この点には注意する
\end{itemize}
...
ローカルに取ってくるコマンド
\item git pullはmasterブランチで次の２つを実行することと同じ意味
\end{itemize}
